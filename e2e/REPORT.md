...
| Layer                                                | Suite                                      | Tests   |
| ---------------------------------------------------- | ------------------------------------------ | ------- |
| folio-api endpoints (Mongo + Redis live)             | `folio-api/tests/integration/`             | 18      |
...
`crates/folio-core/tests/transcription_fixtures.rs` runs the
...
- The multilingual sweep (en-clinical, tr, de, fr, es, action-items)
  ...
  # = cargo test -p folio-core --test transcription_fixtures -- --ignored --nocapture
  ```
- Tests **skip gracefully** (early return + eprintln) when the model
  ...
  Override the model path with `FOLIO_WHISPER_MODEL`.
...
languages. What still can't be automated is _GUI-driving the native
window_ (`tauri-driver` macOS gap) and _live mic capture_ (needs a
