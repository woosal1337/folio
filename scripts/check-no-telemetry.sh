...
#
# Attune's brand promise: "no telemetry, logs are local." Once a
...
for pkg in "${FORBIDDEN[@]}"; do
  for lock in "${LOCKS[@]}"; do
    ...
    if grep -F -q "\"${pkg}\"" "$lock" 2>/dev/null \
      ...
      echo "  Attune is local-first: no telemetry / analytics / crash reporters."
      echo "  See projects/attune/plan/v2-roadmap-multi-agent-consensus.md (finding R11)."
      EXIT=1
    fi
  done
done
