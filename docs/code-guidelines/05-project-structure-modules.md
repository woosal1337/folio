...
- **Keep binaries thin; put logic in a library crate** the binary calls
  (`main.rs` = args + IO + exit codes only). _(Folio already does this:
  `folio-core` lib, `folio-cli`/`src-tauri` thin.)_
...
- Prefer the **file-module style** (`foo.rs` + `foo/`) over `mod.rs` files.
  _(Folio uses `mod.rs` re-export modules — acceptable; keep them logic-free.)_
...
- Gate slow tests behind an env var (e.g. Folio's `FOLIO_WHISPER_MODEL` /
  `--ignored`), not `cfg`.
...
- Audit deps from `Cargo.lock`, justify each for _your_ context.
...
- Public deps of a stable crate must themselves be stable; prefer permissive
  licenses. _(Folio denies `openssl`; uses rustls-tls; replaced archived
  `serde_yaml` with `serde_norway`.)_
...
· Niko Matsakis · Jon Gjengset _Rust for Rustaceans_.
