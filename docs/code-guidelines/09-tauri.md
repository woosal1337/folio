...
- **Keep commands thin and async** for any IO/blocking work so the UI thread
  never freezes. Attune's canonical shape: clone the needed `State` field, do the
...
- **Every return value and error type must be `serde::Serialize`** (it crosses
  IPC). Attune converts `AttuneError` → `String` at the boundary.
...
- **Never expose raw `fs`/`shell`;** restrict with path-glob scopes. Set a strict
  CSP (`default-src 'self'`). *(Attune canonicalizes every path under an allowed
  root — `CODE_STYLE.md` §8.1.)*
