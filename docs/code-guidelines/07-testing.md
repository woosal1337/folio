...
- Integration tests in top-level `tests/`, one crate per file, no `#[cfg(test)]`,
  exercising only the public API. *(Attune: `crates/attune-core/tests/
  transcription_fixtures.rs`.)*
...
- Real Whisper pipeline: `cargo test -p attune-core --test transcription_fixtures
