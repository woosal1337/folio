# Frontend architecture — Attune guidelines
...
| `chrome/` (Attune-specific) | Persistent app chrome (sidebar, drag strip)                  | `src/chrome/`                                             |
...
- **TanStack Query** only if you grow list/detail screens that re-fetch from Rust frequently and benefit from caching/pagination/optimistic updates. Today Attune is below that threshold — keep IPC results in Zustand or component state.
...
Attune's `src/shared/lib/ipc.ts` is the **only** file that should import from `@tauri-apps/api/core`. Every command gets a typed wrapper:
...
Attune currently uses `ts-rs`. It works for one-type-at-a-time exports but has limits:
...
## Attune-specific refactor candidates
