...
- Forbidden in committed Rust code: `println!`, `eprintln!`, `dbg!`. `attune-cli` is the single exception — user-facing CLI output writes to stdout directly.
...
pub async fn do_thing(state: State<'_, AppState>, input: Input) -> Result<Output, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<Output, AttuneError> {
        attune_core::module::do_thing(cloned, input)
    })
    .await
...
3. Delegate to `attune-core` — the command body does not contain domain logic.
4. Map errors at the boundary (`AttuneError → String`); the conversion happens once, here, not deeper.
...
- The `attune://` deep-link handler parses each URL against an explicit allowlist of routes and parameters before navigating. Anything outside the allowlist is logged and dropped.
...
- `attune-core` MUST NOT import Tauri, browser APIs, or anything UI-framework-flavoured. The same code compiles for the CLI and the desktop app.
...
- `src/` (the React layer) MUST NOT perform direct filesystem access via the browser. Anything that needs disk goes through a Tauri command. The React layer also MUST NOT contain Rust-domain logic equivalents (re-implementing what `attune-core` already does).
- `attune-cli` and `src-tauri` both consume `attune-core` and do not call each other.
...
- Zustand stores hold UI state and cached query results only — never authoritative domain state. The Rust `attune-core::storage::*` modules own the durable truth; React stores are a mirror.
- Effects that subscribe to Tauri events MUST return a cleanup function:
  ```ts
  React.useEffect(() => {
    let off: UnlistenFn | undefined;
    listen("transcribe:progress", handler).then((u) => { off = u; });
    return () => { off?.(); };
  }, []);
  ```
...
Code that touches any of these subsystems must cite the matching architecture doc in the PR description, and update the doc in the same PR if the change alters the design:

| Subsystem | Vault doc (canonical) |
|---|---|
| Tauri 2 layout, IPC, capabilities | `projects/attune/architecture/tauri-2-application-architecture.md` |
| Audio capture (cpal, ScreenCaptureKit, VPIO) | `projects/attune/architecture/audio-capture.md` |
| Transcription pipeline (Whisper / OpenAI / hallucination filter) | `projects/attune/architecture/transcription-pipeline.md` |
| Diarization | `projects/attune/architecture/diarization.md` |
| Vault writes (frontmatter schema, atomic writes) | `projects/attune/architecture/vault-write.md` |
| State management (SQLite, migrations) | `projects/attune/architecture/state-management.md` |
| FFI boundary | `projects/attune/architecture/ffi-boundary.md` |
| Privacy and consent | `projects/attune/architecture/privacy-and-consent.md` |
| Meeting detection | `projects/attune/architecture/meeting-detection.md` |
| Calendar integration | `projects/attune/architecture/calendar-integration.md` |
...
- [ ] Layer dependency rule (§9.1) holds: `attune-core` imports no Tauri/browser, `src-tauri` imports no React, `src` imports no Rust-equivalent logic.
