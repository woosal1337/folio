...
Folio ships five copy-paste shortcut recipes that hook into the
`folio://` URL scheme + the `.folio/inbox/` filesystem contract.
Each one is a stock Shortcuts.app pipeline you can rebuild in 30
seconds — no third-party action required.

v2 roadmap finding 074. The fuller App-Intents Swift
...
1. Make sure Folio is running. Settings → Webhooks should show the
   `folio://` scheme registered.
...
```
[Open URL]  folio://recording/start
```

When Folio is running, this brings the app forward and flips the
...
[Run Shell Script]  /bin/zsh
  printf '%s' '{"kind":"stop-recording"}' > "$FOLIO_VAULT/.folio/inbox/stop-$(date +%s).json"
[Open URL]  folio://agent/run?agent_id=summarize
```

`$FOLIO_VAULT` should expand to your vault path (set it in the
shortcut's environment block). Folio polls the inbox and fires
the matching action.
...
[Run Shell Script]   /bin/zsh
  TITLE="$1"
  printf '%s' "{\"kind\":\"create-task\",\"title\":\"$TITLE\"}" > "$FOLIO_VAULT/.folio/inbox/task-$(date +%s).json"
```
...
[Run Shell Script]   /bin/zsh
  folio-cli memory-search "$1" --limit 10
[Show Result]
```

Surfaces the results in a dialog. Requires the `folio-cli` binary
on `PATH` (it ships with the app under `/Applications/Folio.app/
Contents/MacOS/folio-cli` — symlink to `/usr/local/bin` once).

## 5. Get last meeting summary

```
[Run Shell Script]   /bin/zsh
  LATEST=$(folio-cli sessions --limit 1 | jq -r .session_dir)
