...
`#folio` from Reminders into the kanban's inbox column.

. EventKit-bridge native impl is the
...
1. Settings → General → **Sync to Apple Reminders** → ON. Pick the
   target list name (defaults to "Folio").
2. Import the helper Shortcut:
   - Trigger: every 5 minutes
   - For each completed task in the Folio list: drop a JSON file
     into `<vault>/.folio/inbox/` marking it done.
   - For each new uncompleted reminder tagged `#folio`: drop a
     `create-task` JSON into the inbox.

The wire format matches the inbox contract from; Folio's
