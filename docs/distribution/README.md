...
Attune ships through three channels. The direct DMG is the canonical
release; the Mac App Store and Setapp builds are reduced-feature SKUs
of the same Rust core. v2 finding 091 / GET-84.
...
The Cargo workspace exposes three feature flags on `attune-app`:
...
TypeScript side a runtime flag `__ATTUNE_CHANNEL__` (injected by Vite
...
the Cargo features into `attune-app/Cargo.toml`, splitting the Sparkle
...
will gain `GET-84-mas`, `GET-84-setapp`, and `GET-84-feature-matrix`
