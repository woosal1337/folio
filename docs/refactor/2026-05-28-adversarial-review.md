...
7. **`settings_sync` slice has no frontend consumer yet** — intentional
   future work (recordings-sync, task), not accidental dead
...
CODE*STYLE §1.1 says "inline `//` comments inside function bodies are
...
\_intent* (kill WHAT comments, keep WHY) wants — they just lack the
literal `NOTE:` prefix.
