...
- **Agents:**
  - **A** — `crates/folio-core/` Rust audit
...
- §9.1 layer rule: **clean** — no Tauri/browser imports in `folio-core`.
...
| #   | Location                                                                                                                             | Rule                                                                           | Fix                                                                                          |
| --- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------- |
| B1  | `src-tauri/capabilities/default.json:21-24`                                                                                          | §8.4 broad `fs:` scope unjustified                                             | Scope to `$HOME/Documents/Folio/**` or per-window; split `captions.json`.                    |
...
| D7  | `docs/ARCHITECTURE.md`                                    | Materially stale — lists 6 `folio-core` modules; code has 22. Also misses `captions/`, `inbox/`, `onboarding/`, `preferences-window/`.    | Regenerate the layout sections from `lib.rs` and `ls src/features`.                                                                     |
...
- `crates/folio-cli/src/commands/*.rs` — heavy `println!`/`eprintln!` use for debug output; demote debug lines to `tracing::debug!`, keep only user-facing stdout.
- `crates/folio-core/src/audio/devices.rs:93` — `eprintln!` inside core (not CLI) — replace with `tracing::warn!`. (Also captured as A1.)
...
- `docs/integrations/applescript/README.md` — references `Folio.sdef`; file does not exist on disk.
...
- `crates/folio-cli/Cargo.toml` — forces `metal` feature even on Linux/Windows CLI builds; gate by `cfg(target_os = "macos")` or expose as opt-in.
...
- `crates/folio-core/src/lib.rs` — module doc-comment lists 6 modules; `pub mod` exposes 22.
...
- **§1 inline-comment sweep** — 399 in `folio-core` + 96 in `src-tauri` + 263 in `src/` = **758 inline body-comment violations across ~120 files.** Single mechanical pass.
...
| 2026-05-26 | #168 | A1 + A2 + A3 + A5 + A6 — folio-core mechanical fixes.                                                                                                                                                                                                                          |
...
| 2026-05-26 | #184 | P2 quick wins — `.gitignore` covers `*.sqlite`/`*.db`/`*.zst`; `dock_icon.rs` log levels demoted; `folio-core` `lib.rs` doc-comment expanded to 22 modules; `__FOLIO_VERSION__` Vite-time constant replaces the hard-coded `v1.0.0` in the sidebar.                            |
