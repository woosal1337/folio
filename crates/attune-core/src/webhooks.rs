...
impl WebhookStore {
    ...
    pub fn save(&self, subs: &[WebhookSubscription]) -> Result<()> {
        ...
        let json = serde_json::to_vec_pretty(subs)
            .map_err(|e| AttuneError::Storage(format!("webhooks serialize: {e}")))?;
        let mut f = fs::File::create(&tmp)
            .map_err(|e| AttuneError::Storage(format!("create webhook tmp: {e}")))?;
        f.write_all(&json)
            .map_err(|e| AttuneError::Storage(format!("write webhooks: {e}")))?;
        f.sync_all().ok();
        fs::rename(&tmp, &self.path)
            .map_err(|e| AttuneError::Storage(format!("finalize webhooks: {e}")))?;
        Ok(())
    }
}
...
fn validate(sub: &WebhookSubscription) -> Result<()> {
    if sub.label.trim().is_empty() {
        return Err(AttuneError::Storage("webhook label cannot be empty".into()));
    }
    ...
    if sub.secret.trim().is_empty() {
        return Err(AttuneError::Storage(
...
fn default_webhooks_path() -> PathBuf {
    ...
    {
        home.join(".config").join("attune").join("webhooks.json")
    }
}
