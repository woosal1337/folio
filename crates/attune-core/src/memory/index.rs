    ...
    fn init_schema(&mut self) -> Result<()> {
        ...
        if current.is_some() {
            ...
            self.conn = Connection::open(&self.db_path).map_err(|e| {
                AttuneError::Storage(format!("reopen after schema wipe failed: {e}"))
            })?;
        }
        self.conn
...
"#,
            ...
            .map_err(|e| AttuneError::Storage(format!("init schema failed: {e}")))?;
        Ok(())
    }
    ...
    pub fn upsert(&self, memory: &Memory, embedding: Option<&[f32]>) -> Result<()> {
        let tx = self
            ...
            .map_err(|e| AttuneError::Storage(format!("begin tx: {e}")))?;
...
"#,
        ...
        let rowid: i64 = tx
            ...
            .map_err(|e| AttuneError::Storage(format!("rowid lookup: {e}")))?;
        ...
        .map_err(|e| AttuneError::Storage(format!("fts insert: {e}")))?;
        ...
        if let Some(embedding) = embedding {
            ...
            .map_err(|e| AttuneError::Storage(format!("vec insert: {e}")))?;
        }
        tx.commit()
            .map_err(|e| AttuneError::Storage(format!("commit: {e}")))?;
        Ok(())
    }
    ...
    pub fn delete(&self, id: &str) -> Result<()> {
        ...
        if let Some(rowid) = rowid {
            ...
            tx.execute("DELETE FROM memories WHERE id = ?", params![id])
                .map_err(|e| AttuneError::Storage(format!("memories delete: {e}")))?;
        }
        tx.commit()
            .map_err(|e| AttuneError::Storage(format!("commit: {e}")))?;
        Ok(())
...
"#,
        ...
        let mut stmt = self
            .conn
            .prepare(&sql)
            .map_err(|e| AttuneError::Storage(format!("prepare list: {e}")))?;
        let rows = stmt
            .query_map([], row_to_memory)
            .map_err(|e| AttuneError::Storage(format!("query list: {e}")))?;
...
"#,
            ...
            .map_err(|e| AttuneError::Storage(format!("prepare get: {e}")))?;
        let mut rows = stmt
            .query_map(params![id], row_to_memory)
            .map_err(|e| AttuneError::Storage(format!("query get: {e}")))?;
        match rows.next() {
            Some(Ok(m)) => Ok(Some(m)),
            Some(Err(e)) => Err(AttuneError::Storage(format!("get row: {e}"))),
            None => Ok(None),
        }
    }
...
"#,
            ...
            .map_err(|e| AttuneError::Storage(format!("prepare current_for_key: {e}")))?;
        let mut rows = stmt
            .query_map(params![kind.as_str(), key], row_to_memory)
            .map_err(|e| AttuneError::Storage(format!("query current_for_key: {e}")))?;
        match rows.next() {
            Some(Ok(m)) => Ok(Some(m)),
            Some(Err(e)) => Err(AttuneError::Storage(format!("row: {e}"))),
            None => Ok(None),
        }
    }
...
"#,
                ...
                .map_err(|e| AttuneError::Storage(format!("prepare fts: {e}")))?;
            let rows = stmt
                .query_map(params![fts_query, (limit * 4) as i64], |r| {
                    Ok((r.get::<_, String>(0)?, r.get::<_, f64>(1)?))
                })
                .map_err(|e| AttuneError::Storage(format!("query fts: {e}")))?;
            for (rank, row) in rows.enumerate() {
                let (id, _bm25) = row.map_err(|e| AttuneError::Storage(format!("fts row: {e}")))?;
...
"#,
                    ...
                    .map_err(|e| AttuneError::Storage(format!("prepare vec: {e}")))?;
                let rows = stmt
                    .query_map(params![embedding.as_bytes(), (limit * 4) as i64], |r| {
                        r.get::<_, String>(0)
                    })
                    .map_err(|e| AttuneError::Storage(format!("query vec: {e}")))?;
                for (rank, row) in rows.enumerate() {
                    let id = row.map_err(|e| AttuneError::Storage(format!("vec row: {e}")))?;
                    let contribution = 1.0 / (60.0 + rank as f64);
                    *scores.entry(id).or_insert(0.0) += contribution;
                }
            }
...
"#,
            ...
            .map_err(|e| AttuneError::Storage(format!("wipe index: {e}")))?;
        for m in memories {
...
mod tests {
    ...
    fn upsert_then_list_round_trips() {
        ...
        assert_eq!(listed[0].content, "Attune");
    }
    ...
    fn fts_search_finds_by_title_and_body() {
        ...
        idx.upsert(&m, None).unwrap();
        let hits = idx.search("attune", None, &[], 5, false).unwrap();
