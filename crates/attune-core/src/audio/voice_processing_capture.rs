...
/// Streaming VPIO mic capture that writes directly to an
/// [`AudioWavWriter`] via [`StreamingResampler`]. Drop-in replacement
/// for [`crate::audio::mic::MicCapture`] when voice processing is
/// enabled. Owns the AudioUnit + writer + resampler. The render
/// callback runs on the CoreAudio realtime thread; it averages
/// multi-channel frames to mono, resamples to the writer's target
/// rate, and writes — same shape as the cpal mic callback.
pub struct VoiceProcessingMicCapture {
    ...
    /// Channels per frame the unit emits. Usually 1; can be 2 on
    /// some USB interfaces.
    input_channels: u32,
}

impl VoiceProcessingMicCapture {
    /// Build, configure, and start a VPIO unit that writes to
    /// `writer`. Returns once the unit is running.
    ///
    ...
    pub fn start(writer: Arc<AudioWavWriter>, target_sample_rate: u32) -> Result<Self> {
        ...
        // Same minimal setup as the standalone `VoiceProcessingCapture`:
        // enable input bus 1, init, read back the negotiated format.
        // Apple-recommended fuller setups (set device, set format) all
        // returned kAudioUnitErr_FailedInitialization on M-series during
        // phase 1 bring-up; the minimal config is what works.
        let enable: u32 = 1;
        audio_unit
            .set_property(
                kAudioOutputUnitProperty_EnableIO,
        ...
        // Resampler converts VPIO's negotiated rate to the WAV
        // writer's target rate. Mono input + mono output — we collapse
