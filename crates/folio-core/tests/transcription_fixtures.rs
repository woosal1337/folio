...
use folio_core::transcription::local::LocalWhisperTranscriber;
use folio_core::transcription::Transcriber;
...
fn model_path() -> Option<PathBuf> {
    ...
    let pb = PathBuf::from(home).join("Library/Application Support/Folio/models/ggml-large-v3.bin");
...
fn transcript_text(t: &folio_core::transcription::Transcript) -> String {
    t.segments
        .iter()
        .map(|s| s.text.as_str())
        .collect::<Vec<_>>()
...
) -> Option<folio_core::transcription::Transcript> {
