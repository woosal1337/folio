...
pub fn derive_key(passphrase: &[u8], salt: &[u8]) -> Result<[u8; KEY_LEN]> {
    let params = Params::new(64 * 1024, 3, 1, Some(KEY_LEN))
        .map_err(|e| FolioError::Storage(format!("argon2 params: {e}")))?;
    let argon = Argon2::new(Algorithm::Argon2id, Version::V0x13, params);
    let mut out = [0u8; KEY_LEN];
    argon
        .hash_password_into(passphrase, salt, &mut out)
        .map_err(|e| FolioError::Storage(format!("argon2 hash: {e}")))?;
    Ok(out)
}
...
pub fn seal(passphrase: &[u8], plaintext: &[u8]) -> Result<Vec<u8>> {
    ...
    let ciphertext = cipher
        ...
        .map_err(|e| FolioError::Storage(format!("aes-gcm seal: {e}")))?;
...
pub fn open(passphrase: &[u8], envelope: &[u8]) -> Result<Vec<u8>> {
    ...
    cipher
        ...
        .map_err(|e| FolioError::Storage(format!("aes-gcm open: {e}")))
}
