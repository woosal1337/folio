...
const GRANTS_PATH: &str = ".folio/mcp-grants.toml";
const ACCESS_LOG_PATH: &str = ".folio/mcp-access.log";
...
pub fn load_grants(vault_root: &Path) -> crate::error::Result<McpGrants> {
    ...
    let raw = fs::read_to_string(&path)
        .map_err(|e| crate::error::FolioError::Storage(format!("mcp-grants read: {e}")))?;
    toml::from_str(&raw)
        .map_err(|e| crate::error::FolioError::Storage(format!("mcp-grants parse: {e}")))
}
...
pub fn save_grants(vault_root: &Path, grants: &McpGrants) -> crate::error::Result<()> {
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| crate::error::FolioError::Storage(format!("mcp-grants mkdir: {e}")))?;
    }
    let toml = toml::to_string_pretty(grants)
        .map_err(|e| crate::error::FolioError::Storage(format!("mcp-grants serialize: {e}")))?;
    crate::storage::atomic_write::atomic_write(&path, toml.as_bytes())
        .map_err(|e| crate::error::FolioError::Storage(format!("mcp-grants write: {e}")))?;
    Ok(())
}
