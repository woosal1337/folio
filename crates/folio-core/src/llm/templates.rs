...
const TEMPLATE_DIR: &str = ".folio/templates";
...
pub fn parse(input: &str) -> Result<MeetingTemplate> {
    toml::from_str::<MeetingTemplate>(input)
        .map_err(|e| FolioError::Storage(format!("invalid template TOML: {e}")))
}

pub fn render(template: &MeetingTemplate) -> Result<String> {
    toml::to_string_pretty(template)
        .map_err(|e| FolioError::Storage(format!("could not serialise template: {e}")))
}
