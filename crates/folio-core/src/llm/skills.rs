...
const SKILLS_DIR: &str = ".folio/skills";
...
pub fn parse(input: &str) -> Result<Skill> {
    toml::from_str::<Skill>(input)
        .map_err(|e| FolioError::Storage(format!("invalid skill TOML: {e}")))
}

pub fn render(skill: &Skill) -> Result<String> {
    toml::to_string_pretty(skill)
        .map_err(|e| FolioError::Storage(format!("could not serialise skill: {e}")))
}
