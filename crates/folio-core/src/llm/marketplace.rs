...
pub const DEFAULT_REPO: &str = "folio-ai/folio-templates";
...
pub fn parse_index(input: &str) -> crate::error::Result<MarketplaceIndex> {
    toml::from_str::<MarketplaceIndex>(input).map_err(|e| {
        crate::error::FolioError::Storage(format!("invalid marketplace index TOML: {e}"))
    })
}
...
mod tests {
    ...
    fn raw_url_builds_canonical_github_path() {
        let url = raw_url(
            "folio-ai/folio-templates",
            "main",
            "templates/standup/template.toml",
        );
        assert_eq!(
            url,
            "https://raw.githubusercontent.com/folio-ai/folio-templates/main/templates/standup/template.toml"
        );
    }
    ...
    #[test]
    fn default_repo_is_folio_templates() {
        assert_eq!(DEFAULT_REPO, "folio-ai/folio-templates");
    }
}
