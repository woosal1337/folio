...
impl KeyStore {
    pub fn get(provider: ProviderId) -> Result<Option<String>> {
        ...
        match entry.get_password() {
            ...
            Err(e) => Err(FolioError::Keychain(e.to_string())),
        }
    }

    pub fn set(provider: ProviderId, api_key: &str) -> Result<()> {
        if api_key.trim().is_empty() {
            return Err(FolioError::Llm(
        ...
        entry
            ...
            .map_err(|e| FolioError::Keychain(e.to_string()))?;
...
fn entry_for(provider: ProviderId) -> Result<Entry> {
    Entry::new(KEYCHAIN_SERVICE, provider.as_str()).map_err(|e| FolioError::Keychain(e.to_string()))
}
...
mod tests {
    ...
    fn rejects_empty_key() {
        ...
        assert!(matches!(err, FolioError::Llm(_)));
    }
}
