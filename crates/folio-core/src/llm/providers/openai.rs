...
impl OpenAiProvider {
    ...
    fn ensure_egress_allowed(&self) -> Result<()> {
        ...
        crate::cloud_guard::ensure_allowed(host).map_err(|e| FolioError::Llm(e.to_string()))
    }
}
...
impl LlmProvider for OpenAiProvider {
    ...
    async fn test(&self) -> Result<()> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| FolioError::Llm(format!("openai /models request failed: {e}")))?;
    ...
    async fn list_models(&self) -> Result<Vec<ModelInfo>> {
        ...
        let parsed: ModelsResponse = resp
            .json()
            .await
            .map_err(|e| FolioError::Llm(format!("openai /models json decode failed: {e}")))?;
    ...
    async fn chat(&self, request: ChatRequest) -> Result<ChatResponse> {
        ...
        let resp = self
            ...
            .await
            .map_err(|e| {
                FolioError::Llm(format!("openai /chat/completions request failed: {e}"))
            })?;
        let status = resp.status();
        if !status.is_success() {
        ...
        let parsed: ChatCompletionResponse = resp.json().await.map_err(|e| {
            FolioError::Llm(format!("openai /chat/completions json decode failed: {e}"))
        })?;
        let choice = parsed.choices.into_iter().next().ok_or_else(|| {
            FolioError::Llm("openai /chat/completions returned zero choices".to_string())
        })?;
