...
impl VoiceProcessingCapture {
    pub fn new() -> Result<Self> {
        ...
        let mut audio_unit = AudioUnit::new_uninitialized(IOType::VoiceProcessingIO)
            .map_err(|e| FolioError::AudioDevice(format!("VPIO instantiate: {e}")))?;
        ...
        audio_unit
            ...
            .map_err(|e| FolioError::AudioDevice(format!("VPIO enable input: {e}")))?;

        audio_unit
            .initialize()
            .map_err(|e| FolioError::AudioDevice(format!("VPIO initialize: {e}")))?;
        ...
        let negotiated: AudioStreamBasicDescription = audio_unit
            ...
            .map_err(|e| FolioError::AudioDevice(format!("VPIO get format: {e}")))?;
        debug!(
    ...
    pub fn start(&mut self) -> Result<()> {
        if self.running {
            return Err(FolioError::AudioDevice(
        ...
        self.audio_unit
            ...
            .map_err(|e| FolioError::AudioDevice(format!("VPIO callback: {e}")))?;

        self.audio_unit
            .start()
            .map_err(|e| FolioError::AudioDevice(format!("VPIO start: {e}")))?;
        self.running = true;
    ...
    pub fn stop(&mut self) -> Result<Vec<f32>> {
        if self.running {
            self.audio_unit
                .stop()
                .map_err(|e| FolioError::AudioDevice(format!("VPIO stop: {e}")))?;
            self.running = false;
        }
        let mut guard = self
            .samples
            .lock()
            .map_err(|e| FolioError::AudioDevice(format!("VPIO sample lock: {e}")))?;
