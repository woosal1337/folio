...
pub fn apply_aec(mic_path: &Path, system_path: &Path, output_path: &Path) -> Result<AecStats> {
    ...
    if mic_samples.is_empty() || ref_samples.is_empty() {
        return Err(FolioError::Storage("AEC: empty input track".into()));
    }
    ...
    let mut writer = WavWriter::create(output_path, spec)
        .map_err(|e| FolioError::Storage(format!("AEC write: {e}")))?;
    for s in &cleaned {
        writer
            .write_sample(float_to_i16(*s))
            .map_err(|e| FolioError::Storage(format!("AEC write sample: {e}")))?;
    }
    writer
        .finalize()
        .map_err(|e| FolioError::Storage(format!("AEC finalize: {e}")))?;
...
fn read_wav_mono(path: &Path) -> Result<(Vec<f32>, u32)> {
    let mut reader = WavReader::open(path)
        .map_err(|e| FolioError::Storage(format!("AEC read {}: {e}", path.display())))?;
    ...
    let samples_raw: Vec<f32> = match spec.sample_format {
        SampleFormat::Int => reader
            ...
            .map_err(|e| FolioError::Storage(format!("AEC read samples: {e}")))?,
