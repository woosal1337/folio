...
impl StreamingResampler {
    pub fn new(
        ...
        if input_channels == 0 {
            return Err(FolioError::Resampler("input_channels must be > 0".into()));
        }
        ...
        let inner = SincFixedIn::<f32>::new(ratio, 2.0, params, chunk_size, 1)
            .map_err(|e| FolioError::Resampler(format!("init failed: {e}")))?;
    ...
    pub fn process(&mut self, interleaved_input: &[f32]) -> Result<Vec<f32>> {
        ...
        while self.pending.len() >= self.chunk_size {
            ...
            let mut output_frames = self
                ...
                .map_err(|e| FolioError::Resampler(format!("process failed: {e}")))?;
    ...
    pub fn flush(&mut self) -> Result<Vec<f32>> {
        ...
        let mut output_frames = self
            .inner
            .process(&input_frames, None)
            .map_err(|e| FolioError::Resampler(format!("flush failed: {e}")))?;
