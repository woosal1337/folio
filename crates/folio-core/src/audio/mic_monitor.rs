...
impl MicMonitor {
    pub fn start(device_name: Option<&str>) -> Result<Self> {
        ...
        let output_dev = host
            ...
            .ok_or_else(|| FolioError::AudioDevice("no default output device".into()))?;

        let in_cfg = input_dev
            .default_input_config()
            .map_err(|e| FolioError::AudioDevice(format!("input config: {e}")))?;
        let out_cfg = output_dev
            .default_output_config()
            .map_err(|e| FolioError::AudioDevice(format!("output config: {e}")))?;
        ...
        let input_stream = input_dev
            ...
            .map_err(|e| FolioError::AudioDevice(format!("build input stream: {e}")))?;
        ...
        let output_stream = output_dev
            ...
            .map_err(|e| FolioError::AudioDevice(format!("build output stream: {e}")))?;

        input_stream
            .play()
            .map_err(|e| FolioError::AudioDevice(format!("input stream play: {e}")))?;
        output_stream
            .play()
            .map_err(|e| FolioError::AudioDevice(format!("output stream play: {e}")))?;
