...
pub fn concat_wavs(parts: &[std::path::PathBuf], out: &Path) -> Result<()> {
    ...
    if existing.is_empty() {
        return Err(FolioError::Storage("concat_wavs: no input parts".into()));
    }

    let first = hound::WavReader::open(existing[0])
        .map_err(|e| FolioError::Storage(format!("open {}: {e}", existing[0].display())))?;
    let spec = first.spec();
    drop(first);
    ...
    let tmp = out.with_extension("merging.wav");
    {
        ...
        for part in &existing {
            let mut reader = hound::WavReader::open(part)
                .map_err(|e| FolioError::Storage(format!("open {}: {e}", part.display())))?;
            ...
            for sample in reader.samples::<i16>() {
                let s = sample.map_err(|e| FolioError::Storage(format!("read sample: {e}")))?;
                writer
                    .write_sample(s)
                    .map_err(|e| FolioError::Storage(format!("write sample: {e}")))?;
            }
        }
        writer
            .finalize()
            .map_err(|e| FolioError::Storage(format!("finalize {}: {e}", tmp.display())))?;
    }
