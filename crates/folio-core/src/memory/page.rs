...
pub fn delete_page(dir: &Path, memory: &Memory) -> Result<()> {
    ...
    match fs::remove_file(&path) {
        ...
        Err(e) => Err(FolioError::Storage(format!(
...
pub fn parse_page(raw: &str) -> crate::error::Result<Memory> {
    let rest = raw.strip_prefix("---\n").ok_or_else(|| {
        crate::error::FolioError::Storage("missing leading frontmatter delimiter".into())
    })?;
    let end = rest.find("\n---").ok_or_else(|| {
        crate::error::FolioError::Storage("missing trailing frontmatter delimiter".into())
    })?;
    let frontmatter_yaml = &rest[..end];

    let fm: MemoryFrontmatter = serde_norway::from_str(frontmatter_yaml)
        .map_err(|e| crate::error::FolioError::Storage(format!("frontmatter parse error: {e}")))?;

    let kind = MemoryKind::parse(&fm.kind)
        .ok_or_else(|| crate::error::FolioError::Storage(format!("unknown kind: {}", fm.kind)))?;

    let parse_dt = |label: &str, s: &str| -> crate::error::Result<DateTime<Utc>> {
        DateTime::parse_from_rfc3339(s)
            .map(|d| d.with_timezone(&Utc))
            .map_err(|e| crate::error::FolioError::Storage(format!("bad {label}: {e}")))
    };
...
fn render_extra(key: &str, value: &serde_norway::Value) -> crate::error::Result<String> {
    match value {
        ...
        other => {
            ...
            .map_err(|e| crate::error::FolioError::Storage(format!("render_extra: {e}")))?;
            Ok(rendered)
        }
    }
}
...
mod tests {
    ...
    fn sample() -> Memory {
        Memory {
            ...
            content: "Folio.".to_string(),
            evidence: Some("\"I work at Folio now\"".to_string()),
            confidence: 0.92,
