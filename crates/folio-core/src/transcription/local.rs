...
const FOLIO_INITIAL_PROMPT: &str =
    "Folio meeting glossary: Tahir, Yusuf, İbrahim, Ege, Vusal, Azerbaycan, \
...
impl Transcriber for LocalWhisperTranscriber {
    fn transcribe(&self, audio_path: &Path, language_hint: Option<&str>) -> Result<Transcript> {
        ...
        let whisper_context = WhisperContext::new_with_params(
            self.model_path
                .to_str()
                .ok_or_else(|| FolioError::Transcription("non-UTF8 model path".into()))?,
            WhisperContextParameters::default(),
        )
        .map_err(|e| FolioError::Transcription(format!("could not load whisper model: {e}")))?;

        let mut whisper_state = whisper_context
            .create_state()
            .map_err(|e| FolioError::Transcription(format!("whisper state init: {e}")))?;
        ...
        for (idx, range) in ranges.iter().enumerate() {
            ...
            while win_start < range.end {
                ...
                whisper_state
                    ...
                    .map_err(|e| FolioError::Transcription(format!("whisper full(): {e}")))?;
                ...
                let n = whisper_state
                    .full_n_segments()
                    .map_err(|e| FolioError::Transcription(format!("whisper segments: {e}")))?;
                for i in 0..n {
                    let text = whisper_state
                        .full_get_segment_text(i)
                        .map_err(|e| FolioError::Transcription(format!("segment text: {e}")))?;
                    let t0 = whisper_state
                        .full_get_segment_t0(i)
                        .map_err(|e| FolioError::Transcription(format!("segment t0: {e}")))?;
                    let t1 = whisper_state
                        .full_get_segment_t1(i)
                        .map_err(|e| FolioError::Transcription(format!("segment t1: {e}")))?;
    ...
    match spec.sample_format {
        hound::SampleFormat::Float => {
            for sample in reader.into_samples::<f32>() {
                let s = sample
                    .map_err(|e| FolioError::Transcription(format!("wav sample decode: {e}")))?;
                out.push(s);
            }
        }
...
mod tests {
    ...
    fn errors_when_model_missing() {
        ...
        assert!(matches!(err, FolioError::Transcription(_)));
    }
