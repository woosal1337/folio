...
pub fn path_for(memory_dir: &Path) -> PathBuf {
    memory_dir.join(".folio").join(SHOWCASE_FILENAME)
}
...
pub fn write(memory_dir: &Path, showcase: &Showcase) -> Result<PathBuf> {
    let dir = memory_dir.join(".folio");
    fs::create_dir_all(&dir)
        .map_err(|e| FolioError::Storage(format!("create_dir_all {}: {e}", dir.display())))?;
    let path = dir.join(SHOWCASE_FILENAME);
    let body = render(showcase);
    fs::write(&path, body)
        .map_err(|e| FolioError::Storage(format!("write showcase {}: {e}", path.display())))?;
    Ok(path)
}
        ...
        Showcase {
            ...
            entries: vec![
                ShowcaseEntry {
                    ...
                    url: "https://folio.app/s/abc123".into(),
                },
                ShowcaseEntry {
                    title: "Q3 planning offsite".into(),
                    url: "https://folio.app/s/def456".into(),
                },
            ],
        }
    }
