...
pub async fn dispatch(event: WebhookEvent, data: JsonValue) {
    ...
    for sub in subs {
        ...
        match deliver(&sub, &payload).await {
            Ok(status) => {
                info!(target: "folio::webhooks", %label, status, topic = %payload.topic, "delivered")
            }
            Err(e) => {
                warn!(target: "folio::webhooks", %label, topic = %payload.topic, error = %e, "delivery failed")
            }
        }
    }
}
...
async fn deliver(
    ...
    payload: &folio_core::webhooks::WebhookPayload,
) -> Result<String, String> {
    let host = folio_core::cloud_guard::host_of(&sub.url).unwrap_or_default();
    folio_core::cloud_guard::ensure_allowed(host).map_err(|e| e.to_string())?;
    ...
    let resp = client
        ...
        .header("X-Folio-Signature", signature)
        .header("X-Folio-Topic", &payload.topic)
        .body(body)
        .send()
        .await
