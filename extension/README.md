# Folio meeting-context browser extension

Manifest V3 service worker that pings the local Folio app via the
`folio://prepare?url=...&title=...` URL scheme whenever the user
lands on a Zoom / Google Meet / Microsoft Teams tab. Pre-fills the
Record page so the user just hits the big red button.

**Privacy contract**: every payload goes through `folio://`, which
routes to the local Folio app on the user's machine. The
...
The extension fires `folio://prepare?url=<encoded>&title=<encoded>`
twice:
...
The Folio app's deep-link handler (`src/chrome/deep-link-handler.tsx`,
) receives the URL and routes it to the Record page. The
