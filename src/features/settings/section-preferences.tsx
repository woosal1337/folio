...
const LINK_SHARING_OPTIONS: { value: string; label: string; description: string }[] = [
  {
    ...
    description: "Only teammates in your workspace can open a shared link.",
  },
  {
    ...
    <section className="space-y-7">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          App-wide behaviour. Workspace-level settings live under
      ...
      <PreferencesGroup title="Invite teammates to Attune">
        <div className="space-y-2 rounded-lg border border-border bg-card p-4">
          <p className="text-sm text-muted-foreground">
            Workspaces are part of the upcoming v1.0 release. Invite-by-email,
            calendar-derived suggestions, and shared folders ship together.
          </p>
          <div className="flex items-center gap-2 pt-1">
            <PlaceholderButton icon={Copy} disabled>
              Copy link
            </PlaceholderButton>
            <PlaceholderButton icon={Users} disabled>
              Invite by email
            </PlaceholderButton>
          </div>
        </div>
      ...
      <PreferencesGroup title="General">
        <ToggleRow
          ...
          description="A thin indicator on the right edge of the screen while Attune is transcribing."
          checked={settings.live_meeting_indicator}
          onChange={(v) => onChange("live_meeting_indicator", v)}
        />
        <ToggleRow
          icon={LogIn}
          title="Open Attune when you log in"
          description="Attune launches automatically when you sign into macOS."
          checked={settings.open_at_login}
          onChange={(v) => onChange("open_at_login", v)}
        />
        <ToggleRow
          icon={Users}
          title="Move Attune aside in meetings"
          description="When a meeting starts, Attune repositions so you can keep typing notes alongside your conferencing app."
      ...
      <PreferencesGroup title="Appearance">
        <SelectRow
          icon={Palette}
          title="Theme"
          description="Choose how Attune looks. Matches your OS by default."
          value={theme}
          onChange={(v) => {
            onChange("theme", v);
            setTheme(v as Theme);
      ...
      <PreferencesGroup title="Privacy">
        ...
        <ToggleRow
          icon={LinkIcon}
          title="Always open shared links in Attune"
          description="When you click a shared meeting link in your browser, jump into the Attune app instead of opening a web view."
...
function PrivacyRedLineNotice() {
  return (
    <div
      ...
      role="note"
      aria-label="Attune privacy stance"
    >
